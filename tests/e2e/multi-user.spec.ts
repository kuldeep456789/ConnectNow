...
test.describe('Multi-User Meeting Experience', () => {
  test('should show new participant joining in real-time', async ({ browser }) => {
    ...
    const codeElement = page1.locator('text=Security Code').first().locator('..').locator('code').first();
  ...
  test('should update participant count when user leaves', async ({ browser }) => {
    const context1 = await browser.newContext({
      permissions: ['camera', 'microphone'],
    });
    const page1 = await context1.newPage();
  ...
  test('should handle multiple participants in same meeting', async ({ browser }) => {
    const contexts = [];
    const pages = [];
    const userEmails = [
      'user1@example.com',
    ...
    const createBtn = page1.locator('button:has-text("Create Meeting")').first();
    await createBtn.click();
  ...
  test('should maintain connection when one user mutes/unmutes', async ({ browser }) => {
    const context1 = await browser.newContext({
      permissions: ['camera', 'microphone'],
    });
    const page1 = await context1.newPage();
...
test.describe('Meeting Security', () => {
  test('should prevent joining with wrong meeting ID', async ({ page }) => {
    ...
    await page.fill('input[placeholder="Meeting ID"]', '00000000-0000-0000-0000-000000000000');
  ...
  test('should not allow joining same meeting twice simultaneously', async ({ browser }) => {
    const context1 = await browser.newContext({
      permissions: ['camera', 'microphone'],
    });
    const page1 = await context1.newPage();
